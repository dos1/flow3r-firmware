...
static void _header_done(st3m_tar_parser_t *parser) {
    ...
    if (parser->ustar) {
        ...
        size_t prefix_size = strlen((char *)parser->header_current + prefix_offset);
        if (prefix_size > 255) {
            prefix_size = 255;
        }
    ...
    if (parser->on_file_start != NULL) {
        parser->on_file_start(parser->user, name, parser->file_size, parser->type);
    }
...
static int _tinfl_decompress_mem_to_callback(const void *pIn_buf, size_t *pIn_buf_size, tinfl_put_buf_func_ptr pPut_buf_func, void *pPut_buf_user, int flags)
{
    int result = 0;
...
bool st3m_tar_parser_run_zlib(st3m_tar_parser_t *parser, const uint8_t *buffer, size_t bufsize) {
    int ret = _tinfl_decompress_mem_to_callback(buffer, &bufsize, _decompress_cb, parser, TINFL_FLAG_PARSE_ZLIB_HEADER);
    return ret == 1;
}
...
static void _extractor_on_file_start(void *user, const char *name, size_t size, char type) {
    ...
    if (_mkpath(path, 0755) != 0) {
        ESP_LOGW(TAG, "Failed to create parent directory for %s: %s", path, strerror(errno));
    }
...
static void _extractor_on_file_data(void *user, const uint8_t *buffer, size_t bufsize) {
    st3m_tar_extractor_t *extractor = (st3m_tar_extractor_t *)user;
    if (extractor->cur_file == NULL) {
        return;
    }
