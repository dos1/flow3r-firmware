...
void sequencer_run(radspa_t * sequencer, uint16_t num_samples, uint32_t render_pass_id){
    ...
    int16_t bpm = radspa_signal_get_value(bpm_sig, 0, num_samples, render_pass_id);
    int16_t beat_div = radspa_signal_get_value(beat_div_sig, 0, num_samples, render_pass_id);
    ...
    for(uint16_t i = 0; i < num_samples; i++){
        ...
        int16_t sync_in = radspa_signal_get_value(sync_in_sig, i, num_samples, render_pass_id);
        ...
        if(!data->counter){ //event just happened
            for(uint8_t track = 0; track < data->num_tracks; track++){
                ...
                if(type == 32767){
                    ...
                    if(stage_val > 0) data->track_fill[track] = start_trigger(stage_val, &(data->track_prev[track]));
                    if(stage_val < 0) data->track_fill[track] = 0;
                }
            }
        }
