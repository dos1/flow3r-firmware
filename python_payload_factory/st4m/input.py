...
class InputState:
    ...
    """
    def __init__(self, captouch: captouch.CaptouchState, left_button: int, right_button: int) -> None:
    ...
    def gather(cls) -> 'InputState':
        ...
        return InputState(captouch.read(), left_button, right_button)
...
class CaptouchState:
    ...
    def _update(self, ts: int, hr: InputState) -> None:
        for i, petal in enumerate(self.petals):
            petal._update(ts, hr.captouch.petals[i].pressed)
            if i % 2 == 0:
                petal.base._update(ts, hr.captouch.petals[i].pads.base)
                petal.cw._update(ts, hr.captouch.petals[i].pads.cw)
                petal.ccw._update(ts, hr.captouch.petals[i].pads.ccw)
            else:
                petal.base._update(ts, hr.captouch.petals[i].pads.base)
                petal.tip._update(ts, hr.captouch.petals[i].pads.tip)
