    ...
    def on_draw(self, ctx):
        ...
        for i in range(4):
            if self.ct_prev.petals[4-i].pressed:
                dots.append(Dot(48 + groupgap, 40, int((12 * 4 + groupgap)* (1.5 - i)), 0, (0.1,0.1,0.1)))
    ...
    def main_foreground(self):
        ...
        for i in range(4):
            if ct.petals[4-i].pressed:
                for j in range(4):
                    if ct.petals[6+j].pressed and not( self.ct_prev.petals[6+j].pressed):
